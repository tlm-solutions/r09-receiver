...
std::vector<TAP_T> design_resampler_filter(const unsigned interpolation,
  ...
  return firdes::low_pass(interpolation,       /* gain */
                          ...
                          firdes::win_type::WIN_KAISER, beta); /* beta*/
}

template <>
std::vector<gr_complex>
design_resampler_filter<gr_complex>(const unsigned interpolation,
                                    const unsigned decimation,
                                    const float fractional_bw) {
  auto real_taps =
...
rational_resampler<IN_T, OUT_T, TAP_T>::make(unsigned interpolation,
  ...
  return gnuradio::get_initial_sptr(
      new rational_resampler_impl<IN_T, OUT_T, TAP_T>(interpolation, decimation,
                                                      taps, fractional_bw));
}
...
rational_resampler_impl<IN_T, OUT_T, TAP_T>::rational_resampler_impl(
  ...
  if (taps.size() && (d > 1)) {
    GR_LOG_INFO(
        this->d_logger,
        boost::format(
            "Rational resampler has user-provided taps but interpolation (%1%) "
            "and "
            "decimation (%2%) have a GCD of %3%, which increases the "
            "complexity of "
  ...
  if (taps.size() == 0) {
    ...
    interpolation /= d;
    decimation /= d;
    staps = design_resampler_filter<TAP_T>(interpolation, decimation,
                                           fractional_bw);
  } else {
    staps = taps;
  }
...
template <class IN_T, class OUT_T, class TAP_T>
void rational_resampler_impl<IN_T, OUT_T, TAP_T>::set_taps(
    const std::vector<TAP_T> &taps) {
  d_new_taps = taps;
  d_updated = true;
...
void rational_resampler_impl<IN_T, OUT_T, TAP_T>::forecast(
  ...
  int nreqd = std::max(1U, (int)((double)(noutput_items + 1) *
                                 this->decimation() / this->interpolation()) +
                               history() - 1);
